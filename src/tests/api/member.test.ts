...
describe('api/member', () => {
    ...
    it('GET /card/create/:memberId returns 401 when query id fails checkHeader', async () => {
        mockedCheckHeader.mockReturnValue({ valid: false, reason: 'nope', token: '' });
        const res = await request(app).get('/m/card/create/1').query({ id: 'bad' });
        expect(res.status).toBe(401);
        expect(res.body).toEqual({ reason: 'nope' });
        expect(mockedVerify).not.toHaveBeenCalled();
        expect(mockedPdfCtor).not.toHaveBeenCalled();
    });

    it('GET /card/create/:memberId builds PDF with member and president', async () => {
        const token = 'card-token';
        mockedCheckHeader.mockImplementation((hdr: string) =>
            hdr === `Bearer ${token}` ? { valid: true, reason: '', token } : { valid: false, reason: 'bad', token: '' },
        );
        mockedVerify.mockResolvedValue({} as any);
        ...
        mockedBoardList.mockResolvedValue([
            { title: 'Secretary', firstName: 'S', lastName: 'Sec' },
            { title: 'President', firstName: 'Pat', lastName: 'Prez' },
        ] as any);

        const res = await request(app).get('/m/card/create/77').query({ id: token });

        expect(res.status).toBe(200);
        expect(res.headers['content-type']).toMatch(/application\/pdf/);
        expect(res.headers['content-disposition']).toContain('PRAmembershipCard.pdf');
        expect(mockedCheckHeader).toHaveBeenCalledWith(`Bearer ${token}`);
        expect(mockedVerify).toHaveBeenCalledWith(token, 'Member', 77);
        expect(mockedGetMember).toHaveBeenCalledWith('77', 'tenant-test');
        expect(mockedBoardList).toHaveBeenCalledWith('tenant-test', new Date().getFullYear().toString());
        expect(mockedPdfCtor).toHaveBeenCalled();

        const doc = mockedPdfCtor.mock.results[0]?.value as {
            pipe: jest.Mock;
            moveTo: jest.Mock;
            lineTo: jest.Mock;
            image: jest.Mock;
        ...
        expect(doc.image).toHaveBeenCalledWith('src/assets/logo512.png');
        expect(doc.font).toHaveBeenCalledWith('Helvetica');
        expect(doc.font).toHaveBeenCalledWith('src/assets/fonts/GreatVibes-Regular.ttf');
