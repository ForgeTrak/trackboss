    ...
    useEffect(() => {
        async function handleCallback() {
            try {
                ...
                if (window.location.origin === callerOrigin) {
                    // We're already on the right domain (e.g. local dev).
                    // Replace the URL so App.tsx picks up the token from the hash.
                    window.location.replace(`${callerOrigin}/#id_token=${idToken}&state=${stateRaw}`);
                } else {
